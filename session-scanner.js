...
const DEFAULT_HISTORY_LIMIT = Number.parseInt(process.env.TOILET_PI_HISTORY_LIMIT || "200", 10);
const DEFAULT_MESSAGE_LIMIT = Number.parseInt(process.env.TOILET_PI_MESSAGE_LIMIT || "4000", 10);
...
export async function findSessionFile(sessionGuid, sessionDir = getDefaultSessionDir()) {
	if (!sessionGuid) return null;
	const sessions = await scanSessions(sessionDir);
	return sessions.find((session) => session.sessionGuid === sessionGuid)?.sessionFile || null;
}

export async function readSessionSnapshot(sessionFile, options = {}) {
	const resolvedFile = typeof sessionFile === "string" && sessionFile.trim() ? sessionFile : null;
	...
	let model = null;
	const history = [];
	const maxMessages = Math.max(1, Number.parseInt(String(options.maxMessages || DEFAULT_HISTORY_LIMIT), 10) || DEFAULT_HISTORY_LIMIT);
	...
	try {
		for await (const line of rl) {
			...
			if (entry.type === "model_change" && typeof entry.modelId === "string" && entry.modelId.trim()) {
...
function truncateText(text) {
	return text.length > DEFAULT_MESSAGE_LIMIT ? `${text.slice(0, DEFAULT_MESSAGE_LIMIT - 1)}…` : text;
}
